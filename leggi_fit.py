...
sbm.plotcorr(pixs[~cond2].sample,shi[~cond2],cart+'WL_shift.eps',xlabel='sample',ylabel='wl shift (nm)',xlim=[0,260],ylim=[-3,+3])
condshi = (chi < 4) & (col > 2e12)
sbm.plotcorr(pixs[condshi].sample,shi[condshi],cart+'WL_shift_selected.eps',xlabel='sample',ylabel='wl shift (nm)',xlim=[0,260],ylim=[-3,+3])
...
jirfu.stereomap2(lon,lat,col_c,nomefi=cartout+'MAP_h3p_col'+lab+'_image.eps',title='H3+ column [mol/cm2]',polo=polo,min=0.2e12,max=mcol, show=True, xres=npix, image=False, aur_model='both')
sys.exit()
co = err_t < 60
jirfu.stereomap2(lon,lat,col_c,nomefi=cartout+'MAP_h3p_col'+lab+'_errTm60.eps',title='H3+ column [mol/cm2]',polo=polo,min=0.2e12,max=mcol, show=True, xres=npix, image=False,addpoints=True,condpo=co)
co = err_t > 80
jirfu.stereomap2(lon,lat,col_c,nomefi=cartout+'MAP_h3p_col'+lab+'_errTp60.eps',title='H3+ column [mol/cm2]',polo=polo,min=0.2e12,max=mcol, show=True, xres=npix, image=False,addpoints=True,condpo=co)
jirfu.stereomap2(lon,lat,err_cc/col_c,nomefi=cartout+'ERR_h3p_col'+lab+'.eps',title='Relative error H3+ column [mol/cm2]',polo=polo,min=0.,max=1., show=False,xres=npix)

jirfu.stereomap2(lon,lat,col_c,nomefi=cartout+'Npoints_ok'+lab+'.eps',title='Number of measurements',polo=polo,show=True,xres=50,npoints=True)#,addpoints=True)

co = pixs500.emiss_angle < 60
jirfu.stereomap2(lon[co],lat[co],col_c[co],nomefi=cartout+'MAP_h3p_col'+lab+'_emissm60.eps',title='H3+ column [mol/cm2]',polo=polo,min=0.2e12,max=mcol, show=True, xres=npix)
jirfu.stereoplot(lon[co],lat[co],col_c[co],nomefi=cartout+'PLOT_h3p_col'+lab+'_emissm60.eps',title='H3+ column [mol/cm2]',polo=polo,min=0.2e12,max=mcol,show=True)
co = pixs500.emiss_angle > 60
jirfu.stereomap2(lon[co],lat[co],col_c[co],nomefi=cartout+'MAP_h3p_col'+lab+'_emissp60.eps',title='H3+ column [mol/cm2]',polo=polo,min=0.2e12,max=mcol, show=True, xres=npix)
jirfu.stereoplot(lon[co],lat[co],col_c[co],nomefi=cartout+'PLOT_h3p_col'+lab+'_emissp60.eps',title='H3+ column [mol/cm2]',polo=polo,min=0.2e12,max=mcol,show=True)

sys.exit()

cond4 = (pixs.alt_surf < 4e8)
nome = cartout+'MAP_h3p_col_close'+lab+'.eps'
jirfu.stereomap2(lon[cond4],lat[cond4],col_c[cond4],nomefi=nome,title='H3+ column [mol/cm2] -> dist < 4e5 km',polo=polo,min=0.2e12,max=mcol,show=False,xres=npix)
cond4 = (pixs.alt_surf > 4e8)
...
giu = err_t < 100
lab2 = '_err100K'
jirfu.stereomap2(lon[giu],lat[giu],temp[giu],nomefi=cartout+'MAP_h3p_temp'+lab2+lab+'.eps',title='H3+ temperature [K]', polo=polo,min=700.,max=1100, show=False,xres=npix)
jirfu.stereomap2(lon[giu],lat[giu],err_t[giu],nomefi=cartout+'ERR_h3p_temp'+lab2+lab+'.eps',title='Error H3+ temperature [K]', polo=polo, min=0.,max=200., show=False,xres=npix)
giu = err_t < 80
lab2 = '_err80K'
jirfu.stereomap2(lon[giu],lat[giu],temp[giu],nomefi=cartout+'MAP_h3p_temp'+lab2+lab+'.eps',title='H3+ temperature [K]', polo=polo,min=700.,max=1100, show=False,xres=npix)
jirfu.stereomap2(lon[giu],lat[giu],err_t[giu],nomefi=cartout+'ERR_h3p_temp'+lab2+lab+'.eps',title='Error H3+ temperature [K]', polo=polo, min=0.,max=200., show=False,xres=npix)
...
if lch4: jirfu.stereomap2(lon,lat,ch4,nomefi=cartout+'MAP_ch4_col'+lab+'.eps',title='Effective CH4 column [mol/cm2]',polo=polo, show=False,xres=npix)
...
jirfu.stereoplot(lon,lat,col_c,nomefi=cartout+'h3p_col'+lab+'.eps',title='H3+ column [mol/cm2] (corrected)',polo=polo,min=0.2e12,max=mcol)
...
jirfu.stereoplot(lon,lat,temp,nomefi=cartout+'h3p_temp'+lab+'.eps',title='H3+ temperature [K]',polo=polo,min=700.,max=1100)
if lch4: jirfu.stereoplot(lon,lat,ch4,nomefi=cartout+'ch4_col'+lab+'.eps',title='Effective CH4 column [mol/cm2]',polo=polo)
jirfu.stereoplot(lon,lat,pixs.emiss_angle,nomefi=cartout+'emiss_angle'+lab+'.eps',polo=polo,title='Emission angle [deg]')
jirfu.stereoplot(lon,lat,pixs.solar_time,nomefi=cartout+'solar_time'+lab+'.eps',polo=polo,title='Solar time [hr]')
jirfu.stereoplot(lon,lat,off,nomefi=cartout+'offset'+lab+'.eps',polo=polo,title='Offset [W/m2/nm/sr]')
jirfu.stereoplot(lon,lat,pixs.incid_angle,nomefi=cartout+'incid_angle'+lab+'.eps',polo=polo,title='Incidence angle [deg]')
jirfu.stereoplot(lon,lat,pixs.phase_angle,nomefi=cartout+'phase_angle'+lab+'.eps',polo=polo,title='Phase angle [deg]')
...
for cub in cubs:
    ...
    jirfu.stereoplot(lon,lat,col_c[cond],nomefi=nome,title='H3+ column [mol/cm2] (corr) ->'+cub,polo=polo,min=0.2e12,max=2.9e12)
    nome = cartout+cub+'_h3p_temp'+lab+'.eps'
    jirfu.stereoplot(lon,lat,temp[cond],nomefi=nome,title='H3+ temperature [K] ->'+cub,polo=polo,min=700.,max=1100)
    nome = cartout+cub+'_emiss_angle'+lab+'.eps'
    jirfu.stereoplot(lon,lat,gimp.emiss_angle,nomefi=nome,polo=polo,title='Emiss. ang [deg] ->'+cub,min=emin,max=emax)
    nome = cartout+cub+'_offset'+lab+'.eps'
    jirfu.stereoplot(lon,lat,gimp.emiss_angle,nomefi=nome,polo=polo,title='Emiss. ang [deg] ->'+cub,min=omin,max=omax)
