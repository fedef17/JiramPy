...
cart = '/home/federico/Jiram/DATA/JM0003_spettri/aur_N_nadir_70/'
cart500 = '/home/federico/Jiram/DATA/JM0003_SAV_500/nord/'
cartres = cart
...
tot500 = pickle.load(open(cart500+'aur_JM0003_geo500.pic','r'))
tot500 = tot500.view(np.recarray)
...
for cubo,i,j in zip(pixs.cubo,pixs.line,pixs.sample):
	...
	#print(tot500[cond])
	#print(pixs[num])
        for l in range(len(nomi)):
            pi[0][l] = tot500[cond][0][l]
        u+=1
	#print(pi)
    else:
        for l in range(3):
            pi[0][l] = pixs[num][l]
        #for l in range(3,len(nomi)):
        pi[0]['pc_lon'] = np.nan
        pi[0]['pc_lat'] = np.nan
        pi[0]['pg_lon'] = np.nan
        pi[0]['pg_lat'] = np.nan
